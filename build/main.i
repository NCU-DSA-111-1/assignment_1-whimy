# 0 "src/main.c"
# 1 "/home/whimy/workspace/Assignment_1//"
# 0 "<built-in>"
# 0 "<command-line>"
...
int main(void)
{
    ...
    num_neurons = (int*) malloc(4 * sizeof(int));
    memset(num_neurons,0,4 *sizeof(int));
    ...
    *(num_neurons + 3) = 1;
    ...
    for(i=0;i<num_training_ex;i++)
    {
        *(desired_outputs + i) = (float*)malloc(num_neurons[4 -1] * sizeof(float));
    }


    mae = (float *) malloc(num_neurons[4 -1] * sizeof(float));
    memset(mae,0,num_neurons[4 -1]*sizeof(float));
    mse = (float *) malloc(num_neurons[4 -1] * sizeof(float));
    memset(mse,0,num_neurons[4 -1]*sizeof(float));
...
{
    int i=0,j=0;
    lay = (layer*) malloc(4 * sizeof(layer));
    ...
    for(i=0;i<4 -1;i++)
    {
        ...
        for(j=0;j<num_neurons[i];j++)
        {
            for(k=0;k<num_neurons[i+1];k++)
            {
                ...
                lay[i].neu[j].out_weights[k] = ((double)rand())/((double)
# 241 "src/main.c" 3 4
                                                                        2147483647
# 241 "src/main.c"
                                                                                );
            ...
            if(i>0)
            {
                lay[i].neu[j].bias = ((double)rand())/((double)
# 248 "src/main.c" 3 4
                                                              2147483647
# 248 "src/main.c"
                                                                      );
    ...
    for (j=0; j<num_neurons[4 -1]; j++)
    {
        lay[4 -1].neu[j].bias = ((double)rand())/((double)
# 256 "src/main.c" 3 4
                                                                 2147483647
# 256 "src/main.c"
                                                                         );
...
void compute_cost(int i)
{
    ...
    for(j=0;j<num_neurons[4 -1];j++)
    {
        *(mae + j) = fabs(*(*(desired_outputs+i)+j) - lay[4 -1].neu[j].actv);
        *(mse + j) = (*(mae + j)) * (*(mae + j));
        t_mae += *(mae + j);
        t_mse += *(mse + j);
...
void back_prop(int p)
{
    ...
    for(j=0;j<num_neurons[4 -1];j++)
    {
        lay[4 -1].neu[j].dz = (lay[4 -1].neu[j].actv - (*(*(desired_outputs+p)+j))) * (lay[4 -1].neu[j].actv) * (1- lay[4 -1].neu[j].actv);

        for(k=0;k<num_neurons[4 -2];k++)
        {
            lay[4 -2].neu[k].dw[j] = (lay[4 -1].neu[j].dz * lay[4 -2].neu[k].actv);
            lay[4 -2].neu[k].dactv = lay[4 -2].neu[k].out_weights[j] * lay[4 -1].neu[j].dz;
        }

        lay[4 -1].neu[j].dbias = lay[4 -1].neu[j].dz;
    }
